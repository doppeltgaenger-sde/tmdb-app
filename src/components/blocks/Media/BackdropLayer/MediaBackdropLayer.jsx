...
export const MediaBackdropLayer = ({ className, color, backdrop_path }) => {
  const overlay = color ? buildOverlay(color) : undefined;

  const imageUrl = backdrop_path ? `${IMAGE_BASE}${backdrop_path}` : null;
